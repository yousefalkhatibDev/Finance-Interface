...
export default class CreateNewJournalEntry extends Component {
    ...
    componentDidMount() {
        this.state.lines.forEach((l) => {
            this.state.Accounts.push({account: "", description: "", debit: 0, credit: 0})
        })
    }
    addLine() {
        this.setState({
    ...
    submitData(redirect) {
        if(localStorage.getItem("entries")) {
            ...
            localStorage.setItem("entries", JSON.stringify([...entires, {date: this.state.date, reference: this.state.reference, narration: this.state.narration, accounts: this.state.Accounts, id: uuid()}]))
        } else {
            localStorage.setItem("entries", JSON.stringify([{date: this.state.date, reference: this.state.reference, narration: this.state.narration, accounts: this.state.Accounts, id: uuid()}]))
        }
        ...
        <>
            ...
            <div className="body-content">
                <div className="CreateNewJournalEntry-card">
                    <div className="header"><p>Journal Entry</p></div>
                    <div className="body">
                        <div className="input-group-row">
                            <div className="input-container">
                                <label for="date-input">Date</label>
                                <input type="date" id="date-input" name="date-input" onChange={(e) => this.changeState(e, "date")}/>
                            </div>
                            <div className="input-container">
                                <label for="date-input">Reference</label>
                                <div className="merge-inputs">
                                    <span className="merge-inputs-addon">
                                        <input type="checkbox" className='checkbox' onChange={ this.changeIsOptional }/>
                                    </span>
                                    { 
                                    this.state.isOptional === true ?
                                        <input type="text" id="text-input" name="text-input" defaultValue="" placeholder='Optional' onChange={(e) => this.changeState(e, "reference")}/>
                                        :
                                        <input type="text" id="text-input" name="text-input" defaultValue="Automatic" readOnly/>
                                    }
                                </div>
                            </div>
                        </div>
                        <div className="input-group-column">
                            ...
                            <input type="text" id="text-input-1" name="text-input-1" onChange={(e) => this.changeState(e, "narration")}/>
                        </div>
                        {this.state.lines.map((l, i) => {
                                ...
                                <div className="input-group-row">
                                    <div className="input-container">
                                        <label>Account</label>
                                        <select name="Suspense" className="select" onChange={(e) => this.addState(e, "account", i)}>
                                    ...
                                    <div className="input-container">
                                        ...
                                        <input type="text" id="number-input" onChange={(e) => this.addState(e, "description", i)}/>
                                    </div>
                                    }
                                    <div className="input-container">
                                        <label>Debit</label>
                                        <input type="number" id="number-input" onChange={(e) => this.addState(e, "debit", i)}/>
                                    </div>
                                    <div className="input-container">
                                        <label>Credit</label>
                                        <input type="number" id="number-input" onChange={(e) => this.addState(e, "credit", i)}/>
                                    </div>
                                    {
                                    l !== "1" && 
                                    <div className="input-container">
                                        <i class="fa-solid fa-xmark" onClick={() => this.removeLine(l) }></i>
                                    </div>
                                    }   
                                </div>
                            )
                        ...
                        <div className="input-container Credit-Debit-results">
                            <input type="text" id="Credit-result" name="Credit-result" value={"0"} readonly />
                            <input type="text" id="Debit-result" name="Debit-result" value={"0"} readonly />
                        </div>
                        <button className="button" onClick={this.addLine}>Add line</button>
                        <div className="input-group-row">
                            <input type="checkbox" className='checkbox' onChange={ this.changeShowDesc }/>
